...
This script downloads scans from NUNDA in a directory structure with subject_label/session_label (for longitudinal studies).
...
for label in ${ses_list[@]}; do
  ...
  if [[ -d $base/$slabel/$label && $overwrite -eq 0 && $scantypes == "ALL" ]]; then
    echo -e "$base/$slabel/$label directory exists, assuming data downloaded previously.\n"
    continue
  fi
